...
###########################################################################
    ...
    for j in range(len(filterHigh)-2,-1,-1):
        slu = SLupsample(filterLow[j+1],2,1)
        if len(slu.shape) > 1:
            slu = np.squeeze(slu)
            filterLow[-1] = np.squeeze(filterLow[-1])
    ...
    for j in range(len(filterLow2)-2,-1,-1):
        slu = SLupsample(filterLow2[j+1],2,1)
        if len(slu.shape) > 1:
            slu = np.squeeze(slu)
            filterLow2[-1] = np.squeeze(filterLow2[-1])
...
def SLpadArray(array, newSize):
    ...
    if np.isscalar(newSize):
        ...
        if array.ndim == 1:
            ...
            target_shape = paddedArray[padSizes[0]-idxModifier[0]:padSizes[0]+currSize[0]-idxModifier[0],
                    padSizes[1]:padSizes[1]+currSize[1]+idxModifier[1]].shape

            # Вычисляем разницу между целевой формой и текущей формой массива
            pad_sizes = [(0, ts - s) for s, ts in zip(array.shape, target_shape)]

            # Если текущая форма больше целевой, установите размер обрезки в ноль
            pad_sizes = [(max(0, ps[0]), max(0, ps[1])) for ps in pad_sizes]

            # Добавьте нули к массиву, чтобы достичь целевой формы
            array = np.pad(array, pad_sizes, mode='constant')
